...
const sanitizeMessage = (msg: AgentMessage, dirs: string[]): AgentMessage => {
  ...
  return {
    ...
    toolCalls: msg.toolCalls?.map((tc) => ({
      ...
      output: tc.output
        ? (sanitizeValue(tc.output, active) as Record<string, unknown>)
        : undefined,
    })),
  };
};
...
export class RuntimeController extends EventEmitter {
  ...
  private log: Logger;
  private _pendingDetailEmit = new Map<
    string,
    { detail: SessionDetail; timer: ReturnType<typeof setTimeout> }
  >();
  ...
  async loadSessionMessages(chatId: string): Promise<void> {
    ...
    if (
      summary &&
      !["done", "failed", "aborted", "pending"].includes(summary.status)
    )
      return;

    try {
      const cfg = this._config;
      const rawMessages = await fetchChatMessages(
        cfg.url,
        cfg.apiKey,
        cfg.workspace ?? "",
        cfg.project ?? "",
      ...
      // Silently ignore fetch errors — existing messages stay
    }
  }
  ...
  quit(mode: QuitMode): void {
    ...
    const active = this._state.sessions.filter(
  ...
  private addSessionMessage(
    chatId: string,
    msg: Omit<SessionMessage, "id" | "createdAt"> & { id?: string }
  ): void {
    const detail = this.sessionDetails.get(chatId);
    if (!detail) return;
  ...
  private async restoreChatInner(
    ...
    try {
      apiMessages = await this.radar!.fetchMessages(
        cfg.workspace ?? "",
        cfg.project ?? "",
        chatId,
      );
    ...
    const contextAttachment = apiMessages
      .flatMap((m) => m.attachments ?? [])
      .find((a) => a.type === "context" && a.url);
    ...
    if (componentHash) {
      ...
      const sessionMessages: SessionMessage[] = apiMessages
        .filter((m) => m.role !== "user")
        .map((m) => ({
          id: m._id ?? new mongoose.Types.ObjectId().toString(),
          role: m.role,
      ...
      const detail: SessionDetail = { ...summary, messages: sessionMessages };
      ...
      if (hasChanges) {
        ...
        try {
          ...
          const prUrl = await PrService.createPullRequest(
            restoredWorktreeDir, cfg, issue.solution.gitBranch, prContent.title, prContent.body,
          );
          if (prUrl) {
          ...
          this.radar?.emitAgentChatUpdate({ _id: chatId, contextKey: issue.componentHash, status: "finished" });
  ...
  private async processIssueFromResolvePayload(
  ...
  private async processIssue(
    ...
    const activeCount = this._state.sessions.filter(
      (s) => !["done", "failed", "aborted"].includes(s.status)
    ).length;
    ...
    const history: ConversationMessage[] = apiMessages.map(
    ...
    const { callbacks, state: streamState } = this.makeStreamCallbacks(
      chatId,
      () => this.getDirs(chatId),
      "analyzing"
    );
    ...
    try {
      ...
      if (abortController.signal.aborted) {
        ...
        this.radar?.emitAgentChatUpdate({ _id: chatId, contextKey: issue.componentHash, status: "aborted" });
        return;
      }
      ...
      if (patches.length === 0) {
        ...
        this.radar?.emitAgentChatUpdate({ _id: chatId, contextKey: issue.componentHash, status: "error" });
        return;
      }
      ...
      const stillActive = this._state.sessions.filter(
        (s) => !["done", "failed", "aborted"].includes(s.status)
      ).length;
      ...
      if ((this.quitMode as QuitMode | null) === "after-current") {
        const remaining = this._state.sessions.filter(
          (s) => !["done", "failed", "aborted"].includes(s.status)
        ).length;
        if (remaining === 0) {
          this.disconnect();
    ...
    try {
      ...
      if (debugResult?.debugSessionId) {
        radar?.emitAgentChatUpdate({
          ...
          metadata: { debugSession: { _id: debugResult.debugSessionId } },
        });
      }
      ...
      if (attachment) {
        radar?.emitAgentMessage({ chat: chatId, role: "assistant", content: "", agentName: cfg.name, attachments: [attachment] });
      }
      ...
      if (threshold !== undefined && analysis.fixabilityScore < threshold) {
        ...
        radar?.emitAgentChatUpdate({ _id: chatId, contextKey: issue.componentHash, status: "waitingForUserAction" });
  ...
  private async setupGitWorktree(
    ...
    if (!noGitBranch) {
      ...
      const msg = `Preparing isolated workspace for branch \`${branchName}\`...`
   ...
   */
  private async commitPushAndNotify(
    chatId: string,
    issue: Issue,
    workDir: string,
    branchName: string,
    ...
    this.setState(incrementResolved(this._state));
    this.log("info", `Fix pushed: ${branchName} (${commitSha.slice(0, 7)}) +${codeChanges.additions}/-${codeChanges.deletions}`);
  ...
  private async handleUserMessage(msg: AgentMessage): Promise<void> {
    ...
    try {
      ...
      this.radar?.emitAgentChatUpdate({ _id: chatId, contextKey: context.issue.componentHash, status });
    } catch (err: unknown) {
      const message = AiService.classifyAiError(err);
      ...
      this.radar?.emitAgentChatUpdate({ _id: chatId, contextKey: context.issue.componentHash, status: "error" });
    } finally {
      context.isProcessing = false;
      context.abortController = null;
    }
