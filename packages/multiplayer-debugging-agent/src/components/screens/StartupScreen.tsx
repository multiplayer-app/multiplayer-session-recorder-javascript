...
function canSkip(step: StepId, config: Partial<AgentConfig>): boolean {
  switch (step) {
    ...
    case 'workspace':
      // Skip workspace confirmation when both are known and we have an apiKey
...
export const StartupScreen: React.FC<Props> = ({ initialConfig, profileName, onComplete }) => {
  ...
  useLayoutEffect(() => {
    // Clear before first visible paint of setup UI.
    console.clear()
    setReady(true)
  }, [])

  useLayoutEffect(() => {
    // Clear on step transitions to prevent stale frame artifacts.
    if (!ready) return
    console.clear()
  }, [ready, step])

  useEffect(() => {
    // Clear on resize — without this, old Ink frames stack up visibly (broken logo, duplicate content).
  ...
  const advance = useCallback((updates: Partial<AgentConfig>) => {
    ...
    // Find the next step that can't be skipped
    ...
    // All remaining steps can be skipped — go to connecting
    setStep('connecting')
  }, [config, step])

  const handleConnectingComplete = useCallback((finalConfig: AgentConfig) => {
  ...
  useInput((_, key) => {
    if (!key.escape || step === 'api-key') return
    goBack()
  })
  ...
  // Keep progress on one line inside the fixed-width step panel.
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingBottom={0}>
      ...
      <Box flexDirection="row" gap={1} alignItems="flex-start">
        <Box borderStyle="single" borderColor="gray" paddingX={1} width={STEP_PANEL_WIDTH} flexDirection="column">
          ...
          {visibleSteps.map((s, i) => {
            ...
            return (
              <Text key={s} color={color as any} bold={isCurrent}>
                {marker} {STEP_SHORT[s]}
              </Text>
            )
          })}
          <Box marginTop={0}>
            <Text dimColor>{progressPrefix} </Text>
            <Text color="cyan">{'█'.repeat(filled)}</Text>
            <Text dimColor>{'░'.repeat(progressWidth - filled)}</Text>
          </Box>
        </Box>
        ...
        <Box flexDirection="column" flexGrow={1}>
          ...
          <Box borderStyle="single" borderColor="gray" paddingX={1} marginBottom={1}>
            <Text dimColor>
              ...
              {'\n'}
              Profile {profileName ?? 'default'}  ·  Enter confirm  ·  Esc back
            </Text>
          </Box>

          {step === 'api-key' && (
            <ApiKeyStep config={config} onComplete={advance} />
          )}
          {step === 'workspace' && (
            <WorkspaceStep config={config} onComplete={advance} />
          )}
          {step === 'directory' && (
            <DirectoryStep config={config} onComplete={advance} />
          )}
          {step === 'model' && (
            <ModelStep config={config} onComplete={advance} />
          )}
          {step === 'rate-limits' && (
            <RateLimitsStep config={config} onComplete={advance} />
          )}
          {step === 'connecting' && (
            <ConnectingStep
